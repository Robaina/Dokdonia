...
def plotSystemsAndSubsystemsWebPage(
    clusters,
    cluster_data,
    permutation_results,
    plot_first_N=10,
    ...
    def plot_fun(system_type, cluster_id):
        ...
        if cluster_id in clusters.keys():
            ...
            kdata = {k: v for k, v in p_Data[system_type][cluster_id].items()}
            if len(kdata) > plot_first_N:
                kdata = {k: kdata[k] for k in list(kdata.keys())[:10]}
            plotClusterData(cluster_data, clusters[cluster_id], ax["A"], cluster_id)
            plotKEGGFrequencies(kdata, color=color, axis=ax["B"])
        fig.set_figwidth(20)
        fig.set_figheight(20)
        return fig

    i_fig = StaticInteract(
        plot_fun,
        system_type=DropDownWidget(system_types, description="Level"),
        cluster_id=DropDownWidget(cluster_ids, description="Cluster ID"),
...
def getRepresentationStackedPlotData(p_paths, pvalue_cutoff=1):
    ...
    unique_systems = np.unique(
        [system for c in p_paths["system"].values() for system in c.keys()]
    )
    unique_subsystems = np.unique(
        [subsystem for c in p_paths["pathway"].values() for subsystem in c.keys()]
    )
    ...
    for subsystem in unique_subsystems:
        ...
        rep_subsystems = {extractKoPathwayName(k): v for k, v in rep_subsystems.items()}

    return {"system": rep_systems, "pathway": rep_subsystems}


def plotSystemsAndSubsystemsStacked(
    permutation_results, cluster_colors, img_folder_name
):
    """ """
    plt.rcParams.update({"figure.max_open_warning": 0})
    if img_folder_name is None:
    ...
    def plot_fun(system_type, tpvalue_cutoff):
        ...
        df = pd.DataFrame(
            rep_data[system_type], index=list(p_Data["system"].keys()) + ["No cluster"]
        )
    ...
    i_fig = StaticInteract(
        plot_fun,
        system_type=DropDownWidget(system_types, description="Level"),
        tpvalue_cutoff=RangeWidget(
            min=0.05, max=1, step=0.05, default=0.1, description="p-value cutoff"
        ),
        interact_name=img_folder_name,
    )
    return i_fig
